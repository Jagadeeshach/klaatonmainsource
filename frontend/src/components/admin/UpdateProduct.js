...
import { getAllAdminPolicyForProd } from "../../actions/adminMainActions";
...
export default function UpdateProduct() {
  ...
  const { loading:policyloading, policyAdminDetails = [] } = useSelector((state) => state.adminMainState);

  

  const taxAdminPolicy = policyAdminDetails.length > 0 ? policyAdminDetails[0] : null;

  const profitPerCentt = taxAdminPolicy?.platFormCharge ?? 0;  
  const taxPerCentt = taxAdminPolicy?.percentGst ?? 0;
  const outOfMax = taxAdminPolicy?.outOfCharge && taxAdminPolicy?.outOfCharge !== 0 ? taxAdminPolicy?.outOfCharge : 1; 
  const actualPerCentt = taxAdminPolicy?.strategicCharge ?? 0;
  const categories = taxAdminPolicy?.productCategories ?? [];  
  const topCategories = taxAdminPolicy?.productTopCategories ?? [];
 ...
 useEffect(() => {
  ...
  const submitHandler = (e) => {
    ...
    formData.append("topProdCategory", topCategory);
  ...
  useEffect(() => {
    ...
    dispatch(getAllAdminPolicyForProd);
  ...
  useEffect(() => {
    if (product._id) {
      ...
      setTopCategory(product.topProdCategory);
        ...
        <Fragment>
          <div className="wrapper my-5">
            ...
            >
              ...
              <div className="form-group">
                <label htmlFor="category_field">Top Category</label>
                <select
                  value={topCategory}
