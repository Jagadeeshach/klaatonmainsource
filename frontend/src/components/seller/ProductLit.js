...
import { deleteSellerProduct, getSellerProducts } from "../../actions/productActions";
...
export default function ProductList() {
  ...
  const { sellerProducts = [], loading, error, totalPages } = useSelector(
  ...
  const setProducts = () => {
    ...
    sellerProducts.forEach((product) => {
      data.rows.push({
        ...
        name: product.name,
        price: <p>&#8377; {product.basePrice}</p>,
        stock: product.stock,
        actions: (
          <Fragment>
            <Link to={`/vendor/product/${product._id}`} className="btn btn-primary">
  ...
  useEffect(() => {
    ...
    dispatch(getSellerProducts(page));
